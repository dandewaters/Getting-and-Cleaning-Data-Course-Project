...
This is the ReadMe file for the Getting and Cleaning Data Course Project, which will explain my analysis process
...
Objective #1 - "Merge the training and the test sets to create one data set"
  I merged the activity labels to their corresponding datasets using cbind() and rename the column in both tables to "labels" to avoid any conflict when I merge the testing and training data. I then combine the test and training data using rbind().
...
To facilitate objective #5, I melted all of the features columns, converting the dataset into its narrow form. Every feature column had a measurement and dimension component, so I split those columns into their own variables. The resulting columns are a little messy, so I cleaned them up by removing the parenthesis from the measurement column using gsub() and replacing "std" with "standard deviation" to make them more descriptive. I then casted the feature, measurement, and dimension column to factors. This completes objectives 1-4 to make the first tidy dataset, described below.
...
Objective #5 -"From the data set in step 4, create a second, independent tidy data set with the average of each variable for each activity and each subject"
  I used group_by() to gather groups of each feature, measurement, dimension, and activity, and then I used summarise() to apply the mean to all of the groups, resulting in the final tidy data means.
